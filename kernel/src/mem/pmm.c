...
void pmm_init(uint32_t mem) {
	...
	memset(mem_map, 0xFF, max_blocks/8); // we only need (max_blocks/8 bytes -> max_blocks bits) of memory to store bitmap states (1/0) which corresponds to (used/unused)
}
